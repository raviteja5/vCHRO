...
(defrule good
	...
	(test (>= (+ ?resu ?scho ?rwke ?cdct ?pcr ?coso ?expl ?codtl) 37.5)) ;;if sum >= 37.5
	=>
	(assert (good))
	(printout t "This interviewee is a potential candidate" crlf)
)
...
(defrule poor
	...
	=>
	(assert (poor))
	(printout t "This interviewee is a unqualified candidate" crlf)
)
